...
async def _accept_trade(trade: MuseumTrade, engine, today: date) -> bool:
    ...
    for attempt in range(2):
        ...
        if attempt == 0 and ("密钥" in reason or "认证" in reason or "401" in reason or "signature" in reason.lower()):
            ...
            continue
        # 游戏明确说交易已失效 → 标记拒绝
        if "已完成" in reason or "不存在" in reason or "已拒绝" in reason or "已取消" in reason or "已过期" in reason:
...
async def _match_and_create(account_id: str, engine, today: date, surplus: dict[str, int] = None):
    """匹配并创建交易（surplus/deficit 由调用方从DB传入，已在 sync 中校准）"""
    if not surplus:
        return
